...
static void ipc_wwan_setup(void *ctxt, struct net_device *iosm_dev)
{
...
struct iosm_wwan *ipc_wwan_init(struct iosm_imem *ipc_imem, struct device *dev)
{
	...
	BUILD_BUG_ON(sizeof(struct iosm_netdev_priv) > WWAN_MAX_NETDEV_PRIV);
...
void ipc_wwan_deinit(struct iosm_wwan *ipc_wwan)
{
	...
	for (if_id = 0; if_id < ARRAY_SIZE(ipc_wwan->sub_netlist); if_id++) {
		...
		ipc_wwan_dellink(ipc_wwan, priv->netdev);
	}
